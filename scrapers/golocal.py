...
# Header/Get/Rating-Parser sind gemeinsam mit gelbe_seiten/dasoertliche/elfacht.py — siehe
...
def _scrape_query(region, branche, on_lead, stop_event, max_per, BS4):
    logger.scrape("golocal", f"Scanne: {branche} | {region}")
    ...
    if not entries:
        logger.warn("golocal", f"Keine Ergebnisse: {region}/{branche}")
    ...
    for art in entries:
        ...
        # Bewertung + Anzahl (golocal hat oft Bewertungen)
        ...
        if lead_id:
            lead["id"] = lead_id
            logger.success("golocal", f"Gefunden: {name} ({region})")
            try:
                logger.activity("golocal", "Lead gefunden",
...
# Rating-Parser ist gemeinsam mit gelbe_seiten/dasoertliche/elfacht.py — siehe scrapers/_http.py.
